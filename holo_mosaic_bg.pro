...
for ic = 0, ncubes-1 do begin 
  ...
  if nref_accept gt 0 then begin
    ...
    ; mask psf reference image to areas of interest
    mask_accept = tmpmask
    mask_accept[*,*] = 0
    for j_ref = 0, nref_accept-1 do begin
      mask_accept = mask_accept + circ_mask(tmpmask,pr*xint_accept[j_ref],pr*yint_accept[j_ref],2*pr*maskrad)
    endfor
    mask_accept[where(mask_accept ge 1)] = 1
    mask_reject = tmpmask
  ...
  delta_im = delta_im * mask_reject * mask_accept
  HDELTA = FFT(delta_im)*(nax1*nax2)
  ...
  ; ################
     ...
     psfim = CREBIN(im,pr*nax1,pr*nax2,/TOTAL) * mask_reject * mask_accept
...
;    this helps to avoid rare problems with individual bright pixels
     ...
     psfcen = centroid(circ_mask(psf,nax1/2,nax2/2,maskrad))
     ...
     if (debug gt 0) then begin
       ...
       writefits, tmpdir + 'mask_reject.fits', mask_reject
       writefits, tmpdir + 'mask_accept.fits', mask_accept
...
;         print, 'P[0]: ' + strn(P[0])
       ...
       sigma_psf = sigma_psf * sqrt(n_bgind - Num_Rej)
       psf = psf - mean_psf  -  nsigma[it+1]*sigma_psf           ; subtract  offset (to avoid depressed regions around bright stars)
       if (nref_accept gt 2) then begin
         psf_sigma = stack_error(stack, WEIGHTS=psf_weights, MASK=psfmasks)
       endif else begin
         noise = sigma_psf
         psf_sigma = psf
         psf_sigma[*,*] = noise
...
;       suppress = where((psf - nsigma[it+1]*sigma_psf)   lt 0, complement=accept,count)
       if (count gt 0) then  psf[suppress] = 0
       psfcen = centroid(circ_mask(psf,boxhw,boxhw,maskrad))  ; get centroid of PSF
       psf = circ_mask(psf,psfcen[0],psfcen[1],maskrad,BORDER=psf_border)
       psf_sigma = circ_mask(psf_sigma,psfcen[0],psfcen[1],maskrad,BORDER=psf_border)
       psfnorm = total(psf)
       psf = psf/psfnorm
       psf_sigma = psf_sigma/psfnorm
       if (debug gt 0) then begin
        writefits, tmpdir + 'im_model.fits', im_model
        writefits, tmpdir + 'diffuse.fits', diffuse
        ...
        writefits, tmpdir + 'psf_sigma_' +strtrim(string(it+1),2) + '.fits', psf_sigma
        endif
     endfor ; end loop over iterative improval
...
; holography algorithm
  ; #####################
    ...
    if (debug gt 0) then STOP
      ...
      print, "Cube number: " + string(i+1) + " no reference stars detected"
    endelse
 ...
 ; output of current total holography image
  ...
  if (withmask) then begin
   ...
   writefits, tmpdir + 'current_support.fits', rcmask, /COMPRESS
  endif
...
if (withmask) then begin
 ...
 writefits, outnam+'_support.fits', rcmask, /COMPRESS
 writefits, outnam + '_expmap.fits', tmpexp, /COMPRESS
endif
writefits, outnam+'.fits', rcim, /COMPRESS
...
if rawout gt 0 then begin
 ...
 if (withmask) then begin
  ...
  writefits, outnam+'_support_raw.fits', rcmask, /COMPRESS
 endif
 writefits, outnam+'_raw.fits', rcim, /COMPRESS
endif
...
for is = 1, nsub do begin
 ...
 writefits, outnam + '_s' + strtrim(string(is),2) + '.fits', rcim, /COMPRESS
endfor
