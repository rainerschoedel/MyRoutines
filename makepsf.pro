...
PRO MAKEPSF, x_ref, y_ref, x_stars, y_stars, f_stars, image, nrad, FOVMASK = thisfov, PSF=psf,  BACKGROUND=background, DEBUG = debug, ITER = iter, MINDIST = mindist, NOISE_PSF = psf_noise, MASKRAD = mrad, UNWEIGHTED=unweighted, TMPDIR = tmpdir, LOCAL_SKY=local_sky
...
;            endif
       ...
       psf_sigma = stack_error(stack, MASK=masks)
...
; ------------------------------------------
  ...
  for iref = 0, nref_accept-1 do begin
   ...
   psf = stack_median(stack, WEIGHTS=psf_weights, MASK=masks) ; always unweighted first PSF estimate
