...
; For holo_full, we must sort the
; exposures into an appropriate order
; and adapt the field masks
SORT_MASK, maskdir, mlist, cube_order
list = list[cube_order]
...
;          print, strn(ic) + ', ' + strn(i) + ', ' + strn(skip) + ', ' + strn(i_j)
       ...
       if (debug gt 0) then begin
          hhh = REAL_PART(FFT((GHQ/H2), /INVERSE))
          rcim = shift(hhh, center+1)
          writefits, tmpdir + 'rcim.fits', rcim
          hhh = REAL_PART(FFT((GHQ/(H2+DH2)), /INVERSE))
          rcim = shift(hhh, center+1)
    ...
    if ((nthis mod out_iter) eq 0 and nthis gt 0) then begin
     hhh = REAL_PART(FFT((numer/denom), /INVERSE))
     rcim = shift(hhh, center+1)
     hhh = REAL_PART(FFT((numer/(denom+errdenom)), /INVERSE))
...
;       STOP
 ...
 ; output of current total holography image and exposure map
  ...
  hhh = REAL_PART(FFT((finumer/(fidenom+fierrdenom)), /INVERSE))
  rcim = shift(hhh, center+1)
  fov = nexp
...
;  writefits, tmpdir + 'fov_eroded.fits', fov, /COMPRESS
  ...
  for i_j = 0, nsub-1 do begin
    hhh = REAL_PART(FFT((numer_jack[*,*,i_j]/(denom_jack[*,*,i_j]+errdenom_jack[*,*,i_j])), /INVERSE))
    rcim = shift(hhh, center+1)
    rcim[invalid] = 0
