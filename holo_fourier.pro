...
;    GM = FFT(thisfov)
;    HM = FFT(delta_im)*(nax1*nax2)
;    GMH = GM/HM
;    GMH_REAL = REAL_PART(GMH)
;    GMH_REAL = sigma_filter(GMH_REAL, 3*pr, N_SIGMA=5., /ITERATE, /ALL_PIXELS)
;    GMH_IM   = IMAGINARY(GMH)
;    GMH_IM = sigma_filter(GMH_IM, 3*pr, N_SIGMA=5., /ITERATE, /ALL_PIXELS)
;    GMH = COMPLEX(GMH_REAL, GMH_IM)
...
;    hhh = REAL_PART(FFT(GMH,/INVERSE))
;    hhh = CREBIN(hhh,nax1,nax2,/TOTAL)
;    psf_mask = shift(hhh, center+1)
...
;      suppress = where(psf lt nsigma[0] * noise, count)        
      ...
      psf = circ_mask(psf,cen1,cen2,maskrad)
      psf = psf/total(psf)
