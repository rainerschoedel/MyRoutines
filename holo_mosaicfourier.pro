...
for ic = 0, ncubes-1 do begin 
  ...
  if nref_accept gt 0 then begin
    ...
    ; mask psf reference image 
    ; Mask bright stars near edges
    ...
    ; Areas around saturated sources can be masked
    ; but I have not seen that this leads to a signifcant improvement
    ; I leave it as an option
    if KEYWORD_SET(sat_mask) then begin
     if (rebfac gt 1) then  sat_mask = CREBIN(sat_mask,nax1,nax2) ; Do not normalize rebinned FOV!
     edgpix = where(sat_mask ne 1)
     if sat_mask[0] gt -1 then sat_mask[edgpix] = 0
     mask_reject = mask_reject * sat_mask
    endif
  ...
  ; ################
     ...
     psfim = CREBIN(im,pr*nax1,pr*nax2,/TOTAL) * mask_reject
...
;    this helps to avoid rare problems with individual bright pixels
 ...
 ;      writefits, tmpdir + 'mask_accept.fits', mask_accept
  ...
  ; ####################################
     ...
     ; Use only the brightest stars, not more than n_ref_max
     n_ref = n_ref_max < nref_accept ; use smaller of two numbers


     psf_weights = intarr(n_ref) ; for optional weighting
      ...
      for it = 0, iter-1 do begin
       ...
       ; ------------------------------------------------------------
      stack = fltarr(boxsize,boxsize,n_ref)
      psfmasks = fltarr(boxsize,boxsize,n_ref)
