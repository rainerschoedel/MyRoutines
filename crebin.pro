function crebin,image,nsout,nlout,total=total
;+
; NAME:
;   FREBIN
;
...
;   by using CUBIC=-0.5 in calls to INTERPOLATE
;    change name to crebin.pro
;    R. Schoedel 21 06 2016
;----------------------------------------------------------------------------
      On_error,2
      compile_opt idl2

      if N_params() LT 1 then begin
           print,'Syntax = newimage = CREBIN(image, nsout, nlout, [/TOTAL])'  
           return,-1
       endif
...
; Expansion by an integral amount
	if ((nsout mod ns) EQ 0) && ((nlout mod nl) EQ 0) then begin
 		...
 		if keyword_set(total) then $
		return,interpolate(image,xindex,CUBIC=-0.5)*sbox else $        
		return,interpolate(image,xindex,CUBIC=-0.5)  
                endif
                yindex = long(lindgen(nlout)/(nlout/nl))
 		if keyword_set(total) then $
		return,interpolate(image,xindex,yindex,/grid,CUBIC=-0.5)*sbox*lbox else $
		return,interpolate(image,xindex,yindex,/grid,CUBIC=-0.5)  
	endif
   endif
	    ns1 = ns-1
	    nl1 = nl-1
